...
Aligner::Aligner(ProgInfo* prog_info, Genome* genome) : m_prog_info(prog_info), m_genome(genome)
{
	...
	while (reverse_iterator.Current() != NULL) {
		Read* reverse_read = reverse_iterator.Current()->Value();

		Genome::ReverseComplement(reverse_read->sequence);
		Utils::StrReverseSelf(reverse_read->quality);
...
void Aligner::AlignOneRead(GenomeRegion* chromosome, SuffixArray* sa, Read* r)
{
	u_int occ = 0;
	...
	*/
	...
	while (chromosome_iterator.Current() != NULL) {
		...
		while (reads_iterator.Current() != NULL) {
			...
			//align forward read
			AlignOneRead(chromosome, &sa, r);
			//align reverse read
			AlignOneRead(chromosome, &sa, r->paired_read);
